...
'''
def telemetry_and_blur_filter(mas_path ,mas_names, mydir_save,blur , genThred):
    normal = 0
    bad = 0
    max_Laplace_dispersion = blur * 10
    print("=====================", max_Laplace_dispersion)
    for i in range(len(mas_path)):
        ...
        if fm > max_Laplace_dispersion:
            ...
            with Image.open(mas_path[i]) as img:
                img.load()
            img.save(mydir_save + "/intermediate data for AP telemetry and blur/" + mas_names[i])
        else:
            bad += 1
...
def start_blur_filter(mydir_save, blur, genThred):
    print("start blur")
    Trava, names = load_a_start_pic_grass()
    print(genThred)
    telemetry_and_blur_filter(Trava, names, mydir_save, blur, genThred)