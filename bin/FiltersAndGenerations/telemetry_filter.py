...
'''
def telemetry_filter(mas_path ,mas_names, sheet, mydir_save, hight , genThred):
    normal = 0
    bad = 0
    max_height = hight
    print("----------------------------------------------------------", max_height)
    ...
    f.close()
def start_telemetry_filter(mydir_telemetry, mydir_img, mydir_save, hight, genThred):
    print("start telemetry")
    Trava, names = load_a_start_pic_grass(mydir_img)
    wb = opxl.load_workbook(mydir_telemetry)
    sheet = wb['Лист1']

    telemetry_filter(Trava, names, sheet, mydir_save,hight, genThred)
